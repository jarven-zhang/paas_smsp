...
namespace smsp
{
    ...
    CMPPProvinceSubmitReq::CMPPProvinceSubmitReq()
    {
        _msgId = 0;
    ...
    UInt32 CMPPProvinceSubmitReq::bodySize() const
    {
        return _msgContent.size() + 147 + CMPPBase::bodySize();
    }

    bool CMPPProvinceSubmitReq::Pack(Writer& writer) const
    {
		...
		snprintf(Fee_terminal_Id,sizeof(Fee_terminal_Id),"%s",_feeTerminalId.c_str());
        char feeCode[7] = {0};
    ...
    bool CMPPProvinceSubmitReq::Unpack(Reader& reader)
    {        
        return false;
    }

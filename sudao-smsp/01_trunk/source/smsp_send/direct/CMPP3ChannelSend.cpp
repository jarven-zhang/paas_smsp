...
CMPP3ChannelSend::CMPP3ChannelSend()
{
    m_pSocket = NULL;
    m_uHeartbeatTick = 0;
...
UInt32 CMPP3ChannelSend::sendSms( smsDirectInfo &SmsInfo )
{
...
void CMPP3ChannelSend::sendCmppShortMessage( smsDirectInfo *pSmsInfo, vector<string>& vecContent, 
...
void CMPP3ChannelSend::sendCmppLongMessage(smsDirectInfo *pSmsInfo, vector<string>& vecContent,
    UInt32 uTotal,UInt32 uMsgTotal, bool isIncludeChinese )
{
    utils::UTFString utfHelper;
    for (std::vector<std::string>::iterator it = vecContent.begin(); it != vecContent.end(); it++)
    {
        ...
        while(len > 0)
        {
            ...
            LogNotice("==cmpp submit==[%s:%s:%d]sequence:%d,uRand[%d].",
                pSmsInfo->m_strSmsId.data(),pSmsInfo->m_strPhone.data(), 
                m_ChannelInfo.channelID,req._sequenceId,uCand);
...
void CMPP3ChannelSend::bindHandler()
{
    _hander[CMPP_CONNECT_RESP] = &CMPP3ChannelSend::onConnResp;
    _hander[CMPP_SUBMIT_RESP] = &CMPP3ChannelSend::onSubmitResp;
    _hander[CMPP_DELIVER] = &CMPP3ChannelSend::onDeliver;
    _hander[CMPP_ACTIVE_TEST_RESP] = &CMPP3ChannelSend::onHeartbeat;
    _hander[CMPP_ACTIVE_TEST] = &CMPP3ChannelSend::onHeartbeatReq;
}

void CMPP3ChannelSend::onHeartbeatReq(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
...
void CMPP3ChannelSend::onSubmitResp(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
    ...
    if (respSubmit.Unpack(reader))
    {
        ...
        if (it != _packetDB.end())
        {
            ...
            LogNotice("==cmpp submitResp== channelid:%d,sequence:%d,channelsmsid:%lu,result:%s,smsid:%s,phone:%s.",
...
void CMPP3ChannelSend::onDeliver(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
...
void CMPP3ChannelSend::respDeliver(UInt32 seq, UInt64 msgId, int result)
{
...
void CMPP3ChannelSend::onHeartbeat(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
...
void CMPP3ChannelSend::sendHeartbeat()
{
    if (!isAlive())
    {
        return;
...
void CMPP3ChannelSend::onConnResp( pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen )
{
...
void CMPP3ChannelSend::init(models::Channel& chan)
{   
    IChannel::init(chan);
...
void CMPP3ChannelSend::conn()
{
    ...
    LogNotice("CMPP3ChannelSend::connect server [%s:%d] accout[%s], channelid[%d]",
...
bool CMPP3ChannelSend::onTransferData()
{
    cmpp3::CMPPBase resp;
    if(NULL == m_pSocket){
        LogError("socket is null!!");
...
void CMPP3ChannelSend::OnEvent(int type, InternalSocket* socket)
{
    if (socket != m_pSocket)
    {
        return;
    ...
    {
        LogDebug("CMPP3ChannelSend::Connected");
        m_iState = CHAN_INIT;
    }
    ...
    {
        LogError("CMPP3ChannelSend::Close.channelid[%d], channelName[%s]", m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
        m_iState = CHAN_CLOSE;
    ...
    {
        LogError("CMPP3ChannelSend::Error.channelid[%d], channelName[%s]", m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
...
void CMPP3ChannelSend::OnTimer()
{
    ...
    else if (true == m_bHasConn)
    {
        ...
        {
            LogWarn("CMPP3ChannelSend heartbeat timeout,channelid[%d], channelName[%s]",

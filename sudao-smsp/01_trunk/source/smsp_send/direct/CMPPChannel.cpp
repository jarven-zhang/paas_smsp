...
/* 协议发送接口*/
UInt32 CMPPChannel::sendSms( smsDirectInfo &SmsInfo )
{
...
/*发送短短信*/
void CMPPChannel::sendCmppShortMessage( smsDirectInfo *pSmsInfo, vector<string>& vecContent, 
...
/*发送长短信*/
void CMPPChannel::sendCmppLongMessage( smsDirectInfo *pSmsInfo, vector<string>& vecContent, UInt32 uTotal, UInt32 uMsgTotal )
{
    utils::UTFString utfHelper;
    int i = 0;
    for (std::vector<std::string>::iterator it = vecContent.begin(); it != vecContent.end(); it++)
...
void CMPPChannel::bindHandler()
{
    _hander[CMPP_CONNECT_RESP] = &CMPPChannel::onConnResp;
    _hander[CMPP_SUBMIT_RESP] = &CMPPChannel::onSubmitResp;
    _hander[CMPP_DELIVER] = &CMPPChannel::onDeliver;
    _hander[CMPP_ACTIVE_TEST_RESP] = &CMPPChannel::onHeartbeat;
    _hander[CMPP_ACTIVE_TEST] = &CMPPChannel::onHeartbeatReq;
}

void CMPPChannel::onHeartbeatReq( pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen )
{
    m_uHeartbeatTick = 0;
...
void CMPPChannel::onSubmitResp(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
...
/* 收到状态报告直接发送到上报线程处理*/
void CMPPChannel::onDeliver( pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen )
{
...
void CMPPChannel::respDeliver(UInt32 seq, UInt64 msgId, int result)
{
...
/* 收到心跳*/
void CMPPChannel::onHeartbeat( pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen )
{
    m_uHeartbeatTick = 0;
    smsp::CMPPHeartbeatResp respHeartbeat;
...
void CMPPChannel::sendHeartbeat()
{
    if (!isAlive())
    {
        return;
...
/*连接响应回来*/
void CMPPChannel::onConnResp(pack::Reader& reader, UInt32 sequenceId, UInt32 packetLen)
{
...
void CMPPChannel::init( models::Channel& chan )
{   
    IChannel::init(chan);
...
void CMPPChannel::conn()
{
    ...
    LogNotice("CMPPChannel::connect server [%s:%d] accout[%s], channelid[%d]", 
...
bool CMPPChannel::onTransferData()
{
    smsp::CMPPBase resp;
    if( NULL == m_pSocket ){
        LogError("socket is null!!");
...
void CMPPChannel::OnEvent( int type, InternalSocket* socket )
{
    if ( socket != m_pSocket )
    {
        return;
    ...
    {
        LogDebug("CMPPChannel::Connected");
        m_iState = CHAN_INIT;
    }
    ...
    {
        LogError("CMPPChannel::Close.channelid[%d], channelName[%s]", 
    ...
    {
        LogError("CMPPChannel::Error.channelid[%d], channelName[%s]",
                  m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
...
void CMPPChannel::OnTimer()
{
    ...
    else 
    {
        ...
        {
            LogWarn("CMPPChannel heartbeat timeout,channelid[%d], channelName[%s]",
                    m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
            reConn();
        }
        else

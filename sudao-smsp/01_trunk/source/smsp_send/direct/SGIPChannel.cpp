...
UInt32 SGIPChannel::sendSms( smsDirectInfo &SmsInfo )
{
    ...
    if ( cnt > 1 && msgTotal == 1 )
    {
        ...
        sendSgipLongMessage(&SmsInfo, contents,cnt,msgTotal);
    }
    else ///short message
    {
        SmsInfo.m_uLongSms = 1;
...
void SGIPChannel::sendSgipShortMessage( smsDirectInfo *pSmsInfo, vector<string>& vecContent, 
...
void SGIPChannel::sendSgipLongMessage(smsDirectInfo *pSmsInfo, vector<string>& vecContent,UInt32 uTotal,UInt32 uMsgTotal)
{
    utils::UTFString utfHelper;
    int i = 0;
    for (std::vector<std::string>::iterator it = vecContent.begin(); it != vecContent.end(); it++)
    {
        ...
        ////LogDebug("sgip long change len[%d], content[%s].", len,(*it).data());
        int MAXLENGTH = 134;
...
void SGIPChannel::bindHandler() 
{
    _hander[SGIP_BIND_RESP] = &SGIPChannel::onConnResp;
    _hander[SGIP_SUBMIT_RESP] = &SGIPChannel::onSubmitResp;
    _hander[SGIP_UNBIND_RESP] = &SGIPChannel::onUnBindResp;
}

void SGIPChannel::onSubmitResp(pack::Reader& reader, SGIPBase& resp)
{
    ...
    if (respSubmit.Unpack(reader))
    {
        ...
        if (it != _packetDB.end())
        {
            ...
            LogNotice("==sgip submitResp== channelid:%d,channelsmsid:%s,result:%s,iSeq:%u,smsid:%s,phone:%s.",
                     m_ChannelInfo.channelID, respSubmit._msgId.data(),result,
                     resp.sequenceId_,pSms->m_strSmsId.data(),
...
void SGIPChannel::onUnBindResp(pack::Reader& reader, SGIPBase& resp)
{
...
void SGIPChannel::onConnResp(pack::Reader& reader, SGIPBase& resp)
{
    m_uHeartbeatTick = 0;
...
void SGIPChannel::init(models::Channel& chan)
{
    IChannel::init(chan);   
    bindHandler();
...
void SGIPChannel::conn() 
{
    ...
    LogNotice("SGIPChannel::connect server [%s:%d] accout[%s]", ip.data(), port,m_ChannelInfo._clientID.data());
...
void SGIPChannel::reConn()
{
    conn();
}

bool SGIPChannel::onTransferData()
{
    SGIPBase resp;
    if(NULL == m_pSocket){
        LogError("socket is null!!");
...
void SGIPChannel::OnEvent(int type, InternalSocket *socket)
{
    if (socket != m_pSocket)
    {
        return;
    ...
    {
        LogError("SGIPChannel::Close.channelid[%d], channelName[%s]", 
    ...
    {
        LogError("SGIPChannel::Error.channelid[%d], channelName[%s]", m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());

...
void SMPPChannel::init(models::Channel& chan)
{
    IChannel::init(chan);
    bindHandler();
    m_ChannelInfo = chan;
...
UInt32 SMPPChannel::sendSms( smsDirectInfo &SmsInfo )
{
    ...
    if (cnt > 1 && msgTotal == 1)
    {
        ...
        sendSmppLongMessage(&SmsInfo, contents,m_ChannelInfo._showsigntype,cnt,msgTotal);
    }
    else ///short message
    {
...
void SMPPChannel::sendSmppLongMessage( smsDirectInfo *pSmsInfo, vector<string>& vecContent,
                         UInt32 uShowSignType, UInt32 uTotal, UInt32 uMsgTotal )
{
    int i = 0;
    ...
    for (std::vector<std::string>::iterator it = vecContent.begin(); it != vecContent.end(); it++)
    {
        ...
        int len = (int)utfHelper.u2u16((*it), sendContent);
        ////LogDebug("s long change len[%d], content[%s].", len,(*it).data());
        int MAXLENGTH = 134;
        ...
        while(len > 0)
        {
            ...
            saveSmsContent(pSmsInfo, strContent, uTotal, index, req._sequenceNum, uMsgTotal);
            index++;
        }
        delete[] chtmp;
        chtmp = NULL;
...
void SMPPChannel::sendSmppShortMessage(smsDirectInfo *pSmsInfo, vector<string>& vecContent, 
                                UInt32 uShowSignType,UInt32 uTotal,UInt32 uMsgTotal, bool isIncludeChinese)
{
    ...
    for (vector<string>::iterator it = vecContent.begin(); it != vecContent.end(); it++)
    {
        ...
        ////LogNotice("smpp short content[%s],seq[%u].", sendContent.data(), req._sequenceNum);
        if(NULL == m_pSocket){
            LogError("socket is null!!");
            return;
        }
        ...
        saveSmsContent( pSmsInfo, (*it),uTotal, i,req._sequenceNum, uMsgTotal );
...
void SMPPChannel::bindHandler()
{
    _hander[BIND_TRANSCEIVER_RESP] = &SMPPChannel::onConnResp;
    _hander[SUBMIT_SM_RESP] = &SMPPChannel::onSubmitResp;
    _hander[ENQUIRE_LINK] = &SMPPChannel::onHeartbeat;
    _hander[ENQUIRE_LINK_RESP] = &SMPPChannel::onHeartbeat;
    _hander[DELIVER_SM] = &SMPPChannel::onDeliver;

}

void SMPPChannel::onSubmitResp(pack::Reader& reader, SMPPBase& resp)
{
...
void SMPPChannel::onDeliver(pack::Reader& reader, SMPPBase& resp)
{
...
void SMPPChannel::respDeliver(SMPPBase& resp)
{
...
void SMPPChannel::onHeartbeat(pack::Reader& reader, SMPPBase& resp)
{
...
void SMPPChannel::sendHeartbeat()
{
    if (!isAlive())
    {
        return;
...
void SMPPChannel::onConnResp(pack::Reader& reader, SMPPBase& resp)
{
...
void SMPPChannel::conn()
{
    ...
    LogNotice("SMPPChannel::connect server [%s:%d] accout[%s]", ip.data(), port,m_ChannelInfo._clientID.data());
...
bool SMPPChannel::onTransferData()
{
    SMPPBase resp;
    if(NULL == m_pSocket){
        LogError("socket is null!!");
...
void SMPPChannel::OnEvent(int type, InternalSocket* socket)
{
    if (socket != m_pSocket)
    {
        return;
    ...
    {
        LogDebug("SMPPChannel::Connected");     
        m_iState = CHAN_INIT;
    }
    ...
    {
        LogError("SMPPChannel::Close.channelid[%d], channelName[%s]", 
    ...
    {
        LogError("SMPPChannel::Error.channelid[%d], channelName[%s]", m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
...
void SMPPChannel::OnTimer()
{
    ...
    else if (true == m_bHasConn)
    {
        ...
        {
            LogWarn("SMPPChannel heartbeat timeout,channelid[%d], channelName[%s]", m_ChannelInfo.channelID, m_ChannelInfo.channelname.data());
            reConn();
        }
        else
        {
    ...
    for (;iter != _packetDB.end();)
    {
        ...
        if ((iNowTime - session->m_pSmsContext->m_lSubmitDate ) > ICHANNEL_SESSION_TIMEOUT_SECS )
        {
            ...
            LogWarn("==smpp submitResp timeout==[%s:%s:%d]_packetDB size:%d.",
                session->m_pSmsContext->m_strSmsId.data(), session->m_pSmsContext->m_strPhone.data(),

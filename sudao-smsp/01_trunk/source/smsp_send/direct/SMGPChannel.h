#ifndef SMGPCHANNEL_H_
#define SMGPCHANNEL_H_
#include "IChannel.h"
...
class SMGPChannel: public IChannel, SocketHandler

{
	typedef void (SMGPChannel::*HANDLER_FUN)(pack::Reader& reader,UInt32 sequenceId, UInt32 packetLen);	//packetLen = length from head
public:
	SMGPChannel();
	virtual ~SMGPChannel();
...
private:
	...
	bool onTransferData();
	void sendSmgpLongMessage(smsDirectInfo *pSmsInfo, vector<string>& vecContent,UInt32 uTotal,UInt32 uMsgTotal);

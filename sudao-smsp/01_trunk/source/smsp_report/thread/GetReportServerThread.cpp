...
void GetReportServerThread::HandleGetReportOrMoReqMsg(TMsg* pMsg,UInt32 uGetType)
{
	...
	LogNotice("request recived, data[%s], iseq[%ld]", pRequest->m_strRequest.data(), pRequest->m_iSeq);
	...
	{
		LogWarn("smsp push report to user, no need to get report.user[%s]", strAccount.data());
	...
	{
		LogWarn("smsp push mo to user, no need to get mo.user[%s]", strAccount.data());
	...
	{
		UInt32 curInterval = time(NULL) - m_ClientReportReqTimes[strAccount];
		if(curInterval < iterAccount->second.m_uGetRptInterval)
		{
			...
			m_ClientReportReqTimes[strAccount] = time(NULL);
			return;
		}
		m_ClientReportReqTimes[strAccount] = time(NULL);
	...
	{
		UInt32 curInterval = time(NULL) - m_ClientMoReqTimes[strAccount];
		if(curInterval < iterAccount->second.m_uGetRptInterval)
		{
			LogWarn("get report too offten, interval[%d], dbGetRptInterval[%d]", curInterval, iterAccount->second.m_uGetRptInterval);
			Response(pRequest->m_iSeq, ERR_CODE_GETREPORT_RETURN_TOO_OFFTEN, ERR_MSG_GETREPORT_RETURN_TOO_OFFTEN, "");
			m_ClientMoReqTimes[strAccount] = time(NULL);
			return;
		}
		m_ClientMoReqTimes[strAccount] = time(NULL);
		RedisListReq(strAccount,uGetType, pRequest->m_iSeq, iterAccount->second.m_uGetRptMaxSize);
...
void GetReportServerThread::Response(UInt64 iSeq, int iCode, string strMsg, string strData)
{
	/*
	Json::Value tmpValue;
	tmpValue["code"] = Json::Value(iCode);
	tmpValue["msg"] = Json::Value(strMsg);
	tmpValue["data"] = Json::Value(strData);

	Json::FastWriter fast_writer;
	string strResponse = fast_writer.write(tmpValue);
	*/
...
*/
string GetReportServerThread::getDayOffset(int inum)	//eg:today 20160917,int =2. func will return 20160915
{
	char destDt[9];
...
void GetReportServerThread::HandleRedisListReportMsg(TMsg *pMsg)
{
	...
	//
	if(pRedisResp->m_iResult == -1)		//this key has no data
	{
		Response(pRedisResp->m_iSeq, ERR_CODE_GETREPORT_RETURN_OK, ERR_MSG_GETREPORT_RETURN_OK, "");
		return;
	}
	else if(pRedisResp->m_iResult == -2)		//redis link error
	{
	...
	else if(pRedisResp->m_iResult == 0)		//redis response suc
	{
		//将结果应答给用户
	...
	{
		LogWarn("no reason go here, key[%s]", pRedisResp->m_strKey.data());
	}

...
bool CMQIOConsumerThread::InitChannels()
{
	...
	if(false == amqpBasicConsume(SMSP_HTTP_CHANNELID, m_uMqId))
	{
		LogError("amqpBasicConsume fail! ChannelId[%d], MqId[%lu]", SMSP_HTTP_CHANNELID, m_uMqId);
		return false;
	}
	return true;
}
	...
	else
	{
		...
		else
		{
			...
			if(itrMq != m_mapMqConfig.end())
			{
				...
				SetChannelSpeedCtrl(SMSP_HTTP_CHANNELID, stuChannelSpeedCtrl);
				return true;
			}
			else
			{
...
void CMQIOConsumerThread::HandleComponentRefMqRep(TMsg * pMsg)
{
	...
	else
	{
		if(itor->second.m_uMqId != m_uMqId)
		{
			...
			//再订阅
			if(false == amqpBasicConsume(SMSP_HTTP_CHANNELID, itor->second.m_uMqId))
			{
...
void CMQIOConsumerThread::HandleQueueUpdateRep(TMsg* pMsg)
{
	...
	{
		LogNotice("http RtAndMo mq id has changed,oldMqid:%lu, newMqid:%lu.", 
			m_uMqId, m_componentConfigInfo.m_uMqId);
		
		amqp_rpc_reply_t rResult = amqp_channel_close(m_mqConnectionState, SMSP_HTTP_CHANNELID, AMQP_REPLY_SUCCESS);
		if (AMQP_RESPONSE_NORMAL != rResult.reply_type)
		{
			LogError("amqp_channel_close fail! ChannelId[%d]", SMSP_HTTP_CHANNELID);
		}

		if(false == amqpBasicConsume(SMSP_HTTP_CHANNELID, m_componentConfigInfo.m_uMqId))
		{
			LogError("amqpBasicConsume fail! ChannelId[%d], MqId[%lu]", SMSP_HTTP_CHANNELID, m_componentConfigInfo.m_uMqId);
		}
		else
		{
			LogNotice("amqpBasicConsume success! ChannelId[%d], MqId[%lu]", SMSP_HTTP_CHANNELID, m_componentConfigInfo.m_uMqId);
		}
	}
...
{
	LogNotice("==http== get one report or mo message data:%s.",strData.data());

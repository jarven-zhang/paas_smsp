...
void GetBalanceServerThread::HandleMsg(TMsg* pMsg)
{
    ...
    switch (pMsg->m_iMsgType)
    {
        ...
        {
            DbUpdateReqHttp<SysAuthenticationMap>* pReq = (DbUpdateReqHttp<SysAuthenticationMap>*)pMsg;
...
void GetBalanceServerThread::HandleReportReciveReqMsg(TMsg* pMsg)
{
    ...
    LogNotice("get balance request recived, seq[%lu]", pRequest->m_iSeq);
    ...
    //check interval
    UInt32 curInterval = time(NULL) - m_mapClientReqTimes[strAccount];
    ...
    if(curInterval < m_uGetBlcTimeIntva)
    {
        ...
        m_mapClientReqTimes[strAccount] = time(NULL);
        return;
    }
...
{
    AgentInfoMap::iterator itagent = m_AgentInfoMap.find(uAgentID);
...
void GetBalanceServerThread::handleAuthentication(TMsg* pMsg)
{
    ...
    ////////////////////////////////////////////////////////////////////////
    LogNotice("network authentication request recived, iseq[%ld]", pMsg->m_iSeq);

    string strExpireTimestamp = "", strPubKey = "";
    Int32 iCode = 0;
                                                            ...
                                                            string& strPubKey)
{
    ...
    THTTPRequest* pReq = (THTTPRequest*)pMsg;
    ...
    {
        LogError("%s, Call decrypt failed.", pReq->m_strRequest.c_str());
        return false;
    }

    ////////////////////////////////////////////////////////////////////////////////////////

    std::string serial_num = "", component_type = "", component_md5 = "", timestamp = "";
    ...
    string strClientIp = pReq->m_socketHandle->GetServerIpAddress();

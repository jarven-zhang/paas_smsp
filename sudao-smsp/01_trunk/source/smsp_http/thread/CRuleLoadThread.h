...
#define TIMERSEND_TASKS_RESCAN_TIMERID  6071
...
class TUpdateSmsTestAccontReq: public TMsg
{
public:
    std::set<string> m_SmsTestAccount;
};
...
class TUpdateTimerSendTaskInfoReq : public TMsg
{
public:
    map<string, timersend_taskinfo_ptr_t> m_mapTimersendTaskInfo;
};
...
template<typename T>
class DbUpdateReqHttp : public TMsg
{
public:
    T m_map;
};
...
public:
	...
	void getSmsTestAccount(std::set<std::string>& SmsTestAccount);
	...
	void getAgentInfo(AgentInfoMap& agentInfoMap);
	...
	// 获取系统认证信息
	void getSysAuthentication(SysAuthenticationMap& mapSysAuthentication);
	...
	void getTimersendTaskInfo(map<string, timersend_taskinfo_ptr_t>& mapTimersendTaskInfo);
...
private:
	...
	bool getDBSmsTestAccont(std::set<string>& smsTestAccount);
	...
	bool getDBAgentInfo(AgentInfoMap& agentInfoMap);
	////smsp5.0
	...
	bool getDBTimersendTaskInfo(map<string, timersend_taskinfo_ptr_t>& mapTimersendTaskInfo);
    ...
    void tryResetTimersendRescanTimer(UInt32 uTimersendTaskTblRescanInterval);
	...
	AgentInfoMap m_AgentInfoMap;		//agentid - agentInfo
	...
	map<string,ComponentRefMq> m_componentRefMqInfo;
	SysAuthenticationMap m_mapSysAuthentication;		// 系统认证
	...
	std::string  m_SmsTestAccountLastUpdateTime;
    ...
    std::string  m_strSysAuthenticationUpdateTime;


    //timersend taskinfo
	CThreadWheelTimer *m_pTimersendTaskRescanTimer;
    std::map<std::string, timersend_taskinfo_ptr_t> m_mapTimersendTaskInfo;
	std::string m_TimersendTaskInfoUpdateTime;
    // 以下分钟为单位 
	UInt32 m_uTimersendTaskTblRescanInterval; 
    Int32 m_iTimersendTaskInfoForwardScanInterval;
    Int32 m_iTimersendTaskInfoBackwardScanInterval;

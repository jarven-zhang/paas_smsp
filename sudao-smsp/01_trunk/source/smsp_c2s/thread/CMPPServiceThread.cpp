...
CMPPServiceThread::CMPPServiceThread(const char *name):
    CThread(name)
{
...
CMPPServiceThread::~CMPPServiceThread()
{
    delete m_pSnManager;
}

bool CMPPServiceThread::Init(const std::string ip, unsigned int port)
{
...
void CMPPServiceThread::MainLoop()
{
    WAIT_MAIN_INIT_OVER
...
void CMPPServiceThread::HandleMsg(TMsg* pMsg)
{
    if (NULL == pMsg)
    {
...
void CMPPServiceThread::HandleDeliverReq(TMsg* pMsg)
{
    CTcpDeliverReqMsg* pDel = (CTcpDeliverReqMsg*)pMsg;
    CMPPLinkMap::iterator iterLink = m_LinkMap.find(pDel->m_strLinkId);
    ...
    smsp::CMPPDeliverReq reqDeliver;
...
void CMPPServiceThread::HandleSubmitResp(TMsg* pMsg)
{
    CTcpSubmitRespMsg* pResp = (CTcpSubmitRespMsg*)pMsg;
    CMPPLinkMap::iterator iterLink = m_LinkMap.find(pResp->m_strLinkId);
    if (iterLink == m_LinkMap.end())
    {
...
void CMPPServiceThread::HandleLoginResp(TMsg* pMsg)
{
    ...
    smsp::CMPPConnectResp LoginResp;
    LoginResp._status = pResp->m_uResult;
    LoginResp._version= 0X20;
...
void CMPPServiceThread::HandleCMPPLinkCloseReq(TMsg* pMsg)
{
    CTcpCloseLinkReqMsg *pTLinkCloseReq = (CTcpCloseLinkReqMsg*)pMsg;

    CMPPLinkMap::iterator itLink = m_LinkMap.find(pTLinkCloseReq->m_strLinkId);
    if(itLink != m_LinkMap.end())
    {
        ...
        if(itLink->second->m_CMPPHandler != NULL)
        {
            ...
            LogNotice("clientid:%s,linkId:%s,terminate req, m_CMPPHandler[%p]"
                ,pTLinkCloseReq->m_strAccount.data(),pTLinkCloseReq->m_strLinkId.data(), itLink->second->m_CMPPHandler);
        }
        //减少连接数
...
void CMPPServiceThread::HandleCMPPAcceptSocketMsg(TMsg* pMsg)
{
    ...
    CMPPSocketHandler *pCMPPSocketHandler = new CMPPSocketHandler(this, strLinkId);
    if(false == pCMPPSocketHandler->Init(m_pInternalService, pTAcceptSocketMsg->m_iSocket, pTAcceptSocketMsg->m_address))
    {
        LogError("CMPPServiceThread::procSMServiceAcceptSocket is faield.")
        delete pCMPPSocketHandler;
        return;
    }

    LogNotice("accept one cmpp socket LinkId[%s], pCMPPSocketHandler[%p].",strLinkId.data(), pCMPPSocketHandler);
    CMPPLinkManager* pSession = new CMPPLinkManager();
...
void CMPPServiceThread::HandleCMPPHeartBeatReq(TMsg* pMsg)
{
    TCMPPHeartBeatReq *pHeartBeatReq = (TCMPPHeartBeatReq*)pMsg;

    CMPPLinkMap::iterator it = m_LinkMap.find(pMsg->m_strSessionID);
    if(it == m_LinkMap.end())
    {
...
void CMPPServiceThread::HandleCMPPHeartBeatResp(TMsg* pMsg)
{
    TCMPPHeartBeatResp *pHeartBeatResp = (TCMPPHeartBeatResp*)pMsg;
    CMPPLinkMap::iterator it = m_LinkMap.find(pMsg->m_strSessionID);
    if(it == m_LinkMap.end())
    {
        LogWarn("LinkId[%s] is not find in m_LinkMap", pMsg->m_strSessionID.data());
...
void CMPPServiceThread::HandleMoMsg(TMsg* pMsg)
{
    TReportReceiverMoMsg* pMoMsg = (TReportReceiverMoMsg*)pMsg;

    //search client link, and send delivery request.
    CMPPSocketHandler* CMPPHandler = NULL;
    for (CMPPLinkMap::iterator itLink = m_LinkMap.begin(); itLink != m_LinkMap.end(); ++itLink)
    {
    ...
    if(NULL == CMPPHandler)
    {
        LogError("clientId[%s] find no CMPPHandler.", pMoMsg->m_strClientId.data());
    }
    ...
    else if((NULL == CMPPHandler) && (0 != uNeedMo))
    {
        ...
        string strCmd = "lpush " + strKey + " clientid=" + pMoMsg->m_strClientId +"&ismo=1&destid=" + strTemp + "&phone=" + strPhone +
            "&content=" + Base64::Encode(pMoMsg->m_strContent) + "&usmsfrom=" + Comm::int2str(SMS_FROM_ACCESS_CMPP);
...
void CMPPServiceThread::sendCmppLongMo(string strContent,string strDestId,string strPhone,CMPPSocketHandler* CMPPHandler)
{
    if(NULL == CMPPHandler)
    {
...
void CMPPServiceThread::HandleReportReceiveMsg(TMsg* pMsg)
{
...
void CMPPServiceThread::HandleRedisRspSendReportToCMPP(TMsg* pMsg)
{
    //todo...process mo.
    ...
    LogDebug("submitIds[%s], clientId[%s], ids[%s], m_RedisCmd[%s], connectLink[%s]", submitIds.data(), clientId.data(), strIDs.data(), pRedisResp->m_RedisCmd.c_str(), strConnectLink.data());
    UInt32 submitsize;
    ...
    //search client link, and send delivery request.
    CMPPSocketHandler* CMPPHandler = NULL;
    CMPPLinkMap::iterator it = m_LinkMap.find(strConnectLink);
    if (it != m_LinkMap.end())
    {
    ...
    if(NULL == CMPPHandler)
    {
        LogWarn("clientId[%s] find no CMPPHandler.", clientId.data());
    }
    ...
    if (2 == iteror->second.m_uNeedReport)
    {
            ...
            && (0 != strncmp(itReportMsg->second->m_strReportDesc.data(),"ET",2)))
        {
            ...
            {
                int descLen = itReportMsg->second->m_strReportDesc.length();
                string tempDesc;
    ...
    for(UInt32 i = 0; i < submitsize; i++)
    {
        ...
        else if((NULL == CMPPHandler) && (0 != uNeedReport))
        {
            ...
            string strCmd = "lpush " + strKey + " clientid=" + clientId +"&msgid=" + submitslist[i] +"&status=" + Comm::int2str(reqDeliver._status)
                + "&remark=" + Base64::Encode(reqDeliver._remark) + "&phone=" + reqDeliver._srcTerminalId + "&usmsfrom=" + Comm::int2str(SMS_FROM_ACCESS_CMPP);
...
void CMPPServiceThread::HandleReportGetAgain(TMsg* pMsg)
{
    ...
    else if((NULL == CMPPHandler) && (0 != uNeedReport))
    {
        ...
        string strCmd = "lpush " + strKey + " clientid=" + preq->m_strCliendid +"&msgid=" + preq->m_strMsgid.data() +"&status=" + Comm::int2str(preq->m_istatus)
            + "&remark=" + Base64::Encode(preq->m_strRemark) + "&phone=" + preq->m_strPhone + "&usmsfrom=" + Comm::int2str(SMS_FROM_ACCESS_CMPP);
...
void CMPPServiceThread::HandleMOGetAgain(TMsg* pMsg)
{
...
void CMPPServiceThread::HandleTimerOutMsg(TMsg* pMsg)
{
    ...
    else if(CMPP_HEART_BEAT_TIMER_ID == (UInt32)pMsg->m_iSeq)   //send CMPP heartbeat timer.
    {
        ...
        smsp::CMPPHeartbeatReq reqHeartBeat;
...
//update cmpp report
void CMPPServiceThread::UpdateRecord(string& strSubmitId,string strID, UInt32 uIdentify, string strDate, TReportReceiveToSMSReq *rpRecv)
{
    char sql[1024]  = {0};
...
UInt32 CMPPServiceThread::GetSessionMapSize()
{
    return m_LinkMap.size();
}
UInt32 CMPPServiceThread::getRandom()
{
    m_uRandom++;
    if(m_uRandom >=255)
    {

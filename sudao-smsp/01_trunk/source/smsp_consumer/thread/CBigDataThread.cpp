...
CBigDataThread::CBigDataThread(const char *name)
 : CThread(name), m_pSqlFile(NULL)
{
}

CBigDataThread::~CBigDataThread()
{
}


bool CBigDataThread::Init(const std::string PrefixName, std::string fileDir)
{
...
void CBigDataThread::HandleMsg(TMsg* pMsg)
{
    if (NULL == pMsg)
    {
...
bool CBigDataThread::OpenFile(const std::string PrefixName, const std::string fileDir)
{
    if (NULL == m_pSqlFile)
    {
        tm ptm = {0};
        ...
        char szTime[60] = { 0 };
        snprintf(szTime, 60, "%s_%04d%02d%02d", PrefixName.c_str(), (1900 + ptm.tm_year),
                (1 + ptm.tm_mon), ptm.tm_mday);
        fileName = szTime;
        fullPath = fileDir + "/" + fileName + ".cdr";
...
void CBigDataThread::WriteFile(TMsg* pMsg)
{
    tm ptm = {0};
    time_t now = time(NULL);
    localtime_r((const time_t *) &now,&ptm);
    ...
    TSqlContentReq* req = (TSqlContentReq*)pMsg;
...
void CBigDataThread::NewDayLogFile()
{
    if (m_pSqlFile != NULL)
    {
        fclose(m_pSqlFile);
